...
const md5File  = require("./md5file.js");
...
function compile(rawCode, boardName, config, cb) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (typeof GB.file_md5 === "undefined") {
      GB.file_md5 = { };
...
const compileFiles = function(sources, boardCppOptions, boardcflags, plugins_includes_switch,concurrent = 8) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    let exec = async function(file,cmd) {
      let file_md5 = await md5File(file);
      if (typeof GB.file_md5[file] !== "undefined") {
        if (GB.file_md5[file] === file_md5) {
          log(`Skip ${file} use old file.`)
          return;
        }
      }
      GB.file_md5[file] = file_md5;
      try {
        log("Compiling => " + file);
